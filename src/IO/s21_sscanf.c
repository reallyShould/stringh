...
int s21_sscanf(const char *str, const char *format, ...) {
  ...
  while (*p && *str && back != 2) {
    ...
    if (*p == '%' && *(p + 1)) {
      ...
      if (isdigit(*p)) {  // Парсинг ширины, указанной в строке формата
        width = parsing_width(&p);
      } else
        length =
            parsing_length(&p);  // Парсинг длины, указанной в строке формата
      if (back != 2) {
...
int d_specifier(void *dp, const char **str, int *result, int length,
  ...
  if (!isdigit(**str) || !(*width))
    ...
    if (length == 0) {  // Обработка длины
      ...
      *(int *)dp = (sign == 1) ? h_digit : ~h_digit + 1;
    }
    if ((!isdigit(**str) && (**str != '+' && **str != '-')) && !isspace(**str))
      back = 2;
    (*result)++;
  }
  while (!isspace(**str) && (*width) && (**str != '+' && **str != '-') &&
         **str != '\0')
...
int i_specifier(const char **str) {
  ...
  if (**str == '0' && (*(*str + 1) == 'x' ||
                       *(*str + 1) == 'X'))  // Проверка на систему счислиния
    back = 3;
  else if (**str == '0')
    back = 4;
  else if (isdigit(**str))
